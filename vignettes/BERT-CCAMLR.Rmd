---
title: "Estimating local toothfish biomass in CCAMLR data-poor areas"
...
This vignette provides examples of both methods that are similar in lay-out to the R-markdown developed by the Secretariat for local biomass estimation presented at WG-FSA-17. However, instead of using "real data" we provide simulated datasets for a Research block that targets Antarctic toothfish (i.e. RB_TOA) and Research block that targets Patagonian toothfish (i.e. RB_TOP). Given data are simulated there are no data extracts and processing steps included here, but see paper WG-FSA-17/42 or the R markdown document referred to above for details on these steps.
...
The CCAMLR data necessary for both methods can be provided in the same data extracts, but the CPUE-by-seabed area method require catch, length-weight and tagged fish release data from Research blocks and Reference Areas while the Chapman-mark recapture method requires catch, tagged fish release and recapture data from the Research Block. Data extracts can be requested from the CCAMLR Secretariat (see WG-FSA-16/27 for further details on data extracts) in accordance with CCAMLR data access rules. If data extracts are obtained in csv format from the Secretariat these can be loaded into R using the `load_data` function in the BERT package. 

## Simulated data

Due to the CCAMLR data access rules, the catch, tagged fish release, length-weight and matched tagged fish release and recapture datasets described above were simulated for this vignette to represent data from Research Block and Reference Areas that were similar to the data that would be provided in data extracts. For metadata on the four simulated datasets for Research Blocks see:
...
The weights of released fish are predicted using a log-liner model, that is parameterised using length and weight data collected by observers on retained catch (i.e. the length-weight data from the ASD that the Research block is situated in is used to parameterise this model). 
...
The weights of tagged fish releases were predicted and added to the existing data frames as EST_WEIGHT_KG
...
Where the subscripts $x$ and $r$ denote the parameter from the research block and Reference Area respectively. Different Reference Areas were selected for Research Blocks depending on the target species. WG-SAM-16 agreed the HIMI area (Division 58.5.2) should be used as a Reference Area for Research Blocks that target D.eleginoides (WG-SAM-16, para 2.30) and WG-SAM-17 agreed that SSRUs that are open to fishing in the Ross Sea Region (i.e. SSRUs 881B,881C,881G,881H,881I,881J,881K,881L) should be used as a Reference Area for Research Blocks that target D.mawsoni (para 3.11, WG-SAM-17). 
...
The areas of the research block and the Reference Area ($A_x$,$A_r$) were assumed to be known without error and are therefore not contributing to the uncertainty in the bootstrap. See `?CPUE_seabed`

The uncertainty was quantified by taking quantiles of the $B^i_x$, for example, 95% confidence intervals are the 0.025 and 0.975 quantiles.

## Example of estimating Antarctic toothfish biomass using the CPUE-by-seabed area method
...
The target species for the Research Block needs to be specified 
```{r}
...
The Reference Areas for a Research Block is based on the species that is being targeted. As outlined in the general methods above the Reference Area for Research Blocks where TOA are targeted is the Ross Sea Region (RSR). Hence if the Research Block does not target TOP, then it targets TOA and the the `Ref_area` is "RSR" 
...
If the Reference Area is "RSR" then the Reference area biomass (i.e. `Ref_area_biomass`) is 92693.07 tonnes - as this was the estimated vulnerable biomass from the 2017 stock assessment. The `Ref_area_CV` from the Ross Sea stock assessment for the vulnerable biomass was 0.073
...
In Research Blocks the median CPUE can includes data from the last three seasons in which fishing occurred. Depending on the season for which biomass is being estimated (i.e. $y$) data from all seasons are included if there are less than three seasons, but if there are more than three seasons of data then this is restricted to the last three years. 
...
## Chapman mark-recapture biomass estimaton
...
\\
j>1, n_{j-1}e^{-(f+M)}+r_j(1-t)e^{-(f+M)}-m_j
\end{cases}
$$       



where $r_j$ is the total number of fish released in CCAMLR fishing season $j$, $m_j$ 
...
Bootstrap replicates of the haul data where there were zero tag recaptures were used to estimate biomass ($B^i_j$). If the Petersen method had been used this would have been infinite.

For each bootstrap iteration biomass was calculated as: 
...
## Example of estimating Patagonian toothfish using the Chapman mark-recapture method
...
Specify the target species. In this example is TOP 
...
```
Tagged fish recaptures that have been at liberty for up to 3 years are included
...
Survey years specify the number of years of tagged fish releases to include in the tag release and recapture matrix used in the `multi_release` function that calculates the tagged fish available for recapture 
...
The first column includes the season of release, the second column the number of releases in that season and the subsequent columns are all the years  in which released fish were recaptured. In this example there were four recaptures in 2017 - two of them came from the 2015 release event and the other two from the 2016 release event.
...
Variable $y$ is the survey year in `Survey_est` for which toothfish biomass is estimated.  In this example there are three years of tagged fish releases (i.e. 2015, 2016 & 2017), and `Survey_est` just includes 2017. Given, the season_releases is not > 3 years it does not need to be restricted. 
...
With the relevant season(s) of releases and catch data to include in the Research Block specified, the `extract_catch_data_tag_est` function uses the catch data and tagged fish release data from Research Blocks to format the substantial catch and number of tagged fish recaptures per haul for input into the `multi_release` Chapman estimation function. For more information see ?`extract_catch_data_tag_est`
...
Because there are very few recaptures per haul the first six hauls show recaptures, but the sum across hauls shows there are two recaptures that were released in 2015 and two recaptures that were released in 2016 
...
The `multi_release` and `single_release` functions that are used to apply the Chapman mark-recapture biomass estimation method require a list of parameter values and specifications that are passed to these functions in tag_pars. These parameters include a mean fish weight (`mean_wt`) which is set to zero as the mean fish weight is not included in the Chapman biomass estimation formula agreed at WG-SAM-16, the method to be used (i.e. Chapman), the units of the catch (kg), the Ricker type of fishery (which in this case is type 1) and a number of other parameters i.e. natural mortality (`nat_mort`), tagging mortality (`tag_mort`), tag reporting rate (`reporting`), tag shedding (`chronic_shed`) that can vary in relation to the `target_species`. For further details on parameter values for each species see above and for more detailed explanations of parameters see ?`multi_release` or ?`single_release`
...
## Plot biomass estimates from the two different methods with confidence intervals

The ggplot2 package provides nice plots 
